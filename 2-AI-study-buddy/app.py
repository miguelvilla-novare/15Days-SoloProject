...
def main():
    ...
    with st.sidebar:
        ...
        if "generating_flashcards" not in st.session_state:
            st.session_state.generating_flashcards = False  # ✅ New flag for flashcard generation
        ...
        if col1.button("Generate Flashcards", key="generate_flashcards_button", disabled="text_chunks" not in st.session_state):
            st.session_state.generating_flashcards = True  # ✅ Mark flashcards as generating
            with st.spinner("Generating Flashcards..."):
                generate_and_store_flashcards(st.session_state.text_chunks, model)
            st.session_state.generating_flashcards = False  # ✅ Reset after completion
    ...
    chat_input_disabled = (
        ...
        "flashcards" in st.session_state or  # ✅ Disable chat when flashcards are displayed
        "quiz_questions" in st.session_state  # ✅ Disable chat when quiz is active
    )
